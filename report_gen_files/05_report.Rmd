...
if (shapiro_res$p.value > 0.05){
  ...
  tmp <- TukeyHSD(obs_aov)

  tukey_df <- data.frame(tmp$Treatment)
  ...
  p <- p + stat_pvalue_manual(tukey_df, label = "p.adj.signif",hide.ns = TRUE ) + theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1))
  p
