...
ioi <- readLines("item_of_interest.csv")
ioi<-as.character(ioi)
obs_vals <- read.table("results/qiime2/diversity/alpha_diversity/observed_features_vector/metadata.tsv", header = TRUE)
...
shannon_vals <- read.table("results/qiime2/diversity/alpha_diversity/shannon_vector/metadata.tsv", header = TRUE)
...
evenness_vals <- read.table("results/qiime2/diversity/alpha_diversity/evenness_vector/metadata.tsv", header=TRUE)
...
evenness_vals$simpson <- as.numeric(as.character(evenness_vals$pielou_evenness))
...
evenness_stat <- data.frame(evenness[c('id',ioi,'pielou_evenness')])

evenness_vals[[ioi]] <- factor(evenness_vals[[ioi]], levels=ioi_ord)

form <- as.formula(paste0("pielou_evenness ~ ", ioi))
kruskal_table <- rstatix::kruskal_test(data = evenness_stat, formula = form)
kruskal_table <- add_significance(kruskal_table, p.col="p",output.col = "p.signif",symbols =c("****", "***", "**", "$*$", "ns"))
kable(kruskal_table) %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"))
...
dunn_table <- rstatix::dunn_test(data = evenness_stat, formula = form, p.adjust.method = "holm") 
dunn_table<- add_significance(dunn_table, p.col="p.adj",output.col = "p.adj.signif",symbols =c("****", "***", "**", "$*$", "ns"))
kable(dunn_table) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
faith_pd_vals <- read.table("results/qiime2/diversity/alpha_diversity/faith_pd_vector/metadata.tsv", header=TRUE)
#TODO remove if not needed
