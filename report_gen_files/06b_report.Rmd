...
bray_ord <- plot_ordination(cycle_1, cycle_1.ord, type="samples", color=ioi) + ggtitle("NMDS ordination of Bray curtis")
bray_ord_poly <- bray_ord + geom_polygon(aes(fill=ioi)) 

ggsave("bray_curtis_pcoa_ordination_polygon.png", plot=bray_ord_poly, path=outdir)
ggsave("bray_curtis_pcoa_ordination.png", plot=bray_ord, path=outdir)
```
...
jaccard <- qiime2R::read_qza("jaccard_pcoa_results.qza")
shannon<-qiime2R::read_qza("shannon_vector.qza")
...
metadata<-read_q2metadata(metadata_file)
metadata[[ioi]] <- factor(metadata[[ioi]], levels=ioi_ord)
jaccard <- qiime2R::read_qza("unweighted_unifrac_pcoa_results.qza")
shannon<-qiime2R::read_qza("shannon_vector.qza")
shannon <- shannon$data %>% rownames_to_column("SampleID")
...
metadata<-read_q2metadata(metadata_file)
metadata[[ioi]] <- factor(metadata[[ioi]], levels=ioi_ord)
jaccard <- qiime2R::read_qza("weighted_unifrac_pcoa_results.qza")
shannon<-qiime2R::read_qza("shannon_vector.qza")
shannon <- shannon$data %>% rownames_to_column("SampleID")
...
#potentially need to add a row that is qiime types
cycle_1 <- qza_to_phyloseq(table_dada2,rooted_tree,taxonomy_file,metadata_file) 
